#!/usr/bin/env python3
"""Contrarian strategy: Inverted median-sign momentum strategy.

This module implements the exact opposite of the median momentum strategy,
inverting all signals to create a contrarian approach.

Three strategy variants are provided (all inverted):
 - ts: inverted time-series sign median (s_t)
 - cs: inverted cross-sectional median on L-period cumulative returns
 - hybrid: inverted hybrid requiring agreement between ts sign and cs direction
...
def compute_s_t(returns, L):
    # CONTRARIAN: INVERTED median of signs of past L returns, using only r_{t-1}..r_{t-L}
...
def cs_positions(cumret):
    # CONTRARIAN: INVERTED cross-sectional median cut
    # Original: +1 if > median, -1 if < median, 0 else
    # Contrarian: -1 if > median, +1 if < median, 0 else
...
def normalize_positions(pos):
    ...
    shorts = shorts.div(-short_sum, axis=0).fillna(0.0) * -1.0
...
def run(L=63, variant='hybrid'):
    ...
    s_t = compute_s_t(returns, L)  # Already inverted
    cumret = compute_cumret(returns, L)
    pos_cs = cs_positions(cumret)  # Already inverted
    pos_ts = s_t
    ...
    if variant == 'cs':
        ...
        # CONTRARIAN HYBRID: require sign agreement between inverted signals
        # Note: both s_t and pos_cs are already inverted, so the logic remains the same
        sign_cum = np.sign(cumret).astype('Int64')
        # Invert the cumret sign for contrarian approach
        sign_cum = -sign_cum
...
if __name__ == '__main__':
    ...
    p = argparse.ArgumentParser(description='Contrarian strategy - Inverted median-sign momentum')
